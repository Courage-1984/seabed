...
function initReveals() {
  ...
  // At >=900px the window never scrolls: .panel-right is the scroll container and
  // scroll events do not bubble out of it, so the backstop needs its own hook.
  document.querySelector('.panel-right')?.addEventListener('scroll', sweep, { passive: true });
...
function initEffect() {
  ...
  // Same scroll container caveat: without this the pull never advances on desktop.
  document.querySelector('.panel-right')?.addEventListener('scroll', onScroll, { passive: true });
  update();
}
