...
function initSlotVideo() {
  // The modal's clip is driven by the dialog's own open/close, not by
  // viewport intersection — it would otherwise decode behind [hidden].
 ...
 * button, Tab is cycled inside the frame, Escape closes, and focus returns
 * to the tile that opened it.
 * ------------------------------------------------------------------ */
function initVideoModal() {
  const opener = document.querySelector('[data-video-open]');
  const modal = document.getElementById('cell-film');
